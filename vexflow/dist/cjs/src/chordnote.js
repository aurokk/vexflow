...
import { isChordStave } from './typeguard';
import { RuntimeError } from './util';
export var SymbolType;
(function (SymbolType) {
    SymbolType[SymbolType["GLYPH"] = 1] = "GLYPH";
    SymbolType[SymbolType["TEXT"] = 2] = "TEXT";
    SymbolType[SymbolType["LINE"] = 3] = "LINE";
})(SymbolType || (SymbolType = {}));
export var SymbolModifier;
(function (SymbolModifier) {
    SymbolModifier[SymbolModifier["NONE"] = 1] = "NONE";
    SymbolModifier[SymbolModifier["SUBSCRIPT"] = 2] = "SUBSCRIPT";
    SymbolModifier[SymbolModifier["SUPERSCRIPT"] = 3] = "SUPERSCRIPT";
})(SymbolModifier || (SymbolModifier = {}));
class ChordNote extends Note {
    static get CATEGORY() {
        return "ChordNote" /* Category.ChordNote */;
    }
    /**
    ...
    constructor(noteStruct, options) {
        super(noteStruct);
        this.chordBlocks = [];
    ...
    setStave(stave) {
        if (!isChordStave(stave)) {
            throw new RuntimeError('InvalidStaveType', 'ChordNote can only be attached to ChordStave instances. Use ChordStave instead of Stave.');
        }
    ...
    checkChordStave() {
        ...
        if (!isChordStave(stave)) {
            throw new RuntimeError('InvalidStaveType', 'ChordNote requires a ChordStave, but a different stave type was attached.');
        }
        return stave;
    }
    /**
    ...
    getSymbolBlock(params = {}) {
        var _a, _b, _c;
        const symbolType = (_a = params.symbolType) !== null && _a !== void 0 ? _a : SymbolType.TEXT;
        const symbolBlock = {
            text: (_b = params.text) !== null && _b !== void 0 ? _b : '',
            symbolType,
            symbolModifier: (_c = params.symbolModifier) !== null && _c !== void 0 ? _c : SymbolModifier.NONE,
            xShift: 0,
            yShift: 0,
            vAlign: false,
            width: 0,
    ...
    updateOverBarAdjustments() {
        const barIndex = this.chordBlocks.findIndex(({ symbolType, glyph }) => symbolType === SymbolType.GLYPH && glyph !== undefined && glyph.code === 'csymDiagonalArrangementSlash');
        if (barIndex < 0) {
            return;
        }
    ...
    getKerningAdjustment(j) {
        ...
        if (currSymbol.symbolType === SymbolType.GLYPH &&
        ...
        if (preKernLower && currSymbol.symbolType === SymbolType.TEXT) {
            ...
            if (ChordNote.upperKerningText.some((xx) => xx === (prevSymbol === null || prevSymbol === void 0 ? void 0 : prevSymbol.text[prevSymbol.text.length - 1]))) {
                adjustment += kerningOffsetPixels / 2;
            }
        }
        return adjustment;
    ...
    draw() {
        ...
        const baseY = this.options.line === 2 && 'getCenterY' in chordStave
            ? chordStave.getCenterY()
