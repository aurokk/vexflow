...
class GlyphOutline {
    ...
    static parse(str) {
        ...
        while (i < parts.length) {
            switch (parts[i++]) {
                case 'm':
                    result.push(0 /* OutlineCode.MOVE */, parseInt(parts[i++]), parseInt(parts[i++]));
                    break;
                case 'l':
                    result.push(1 /* OutlineCode.LINE */, parseInt(parts[i++]), parseInt(parts[i++]));
                    break;
                case 'q':
                    result.push(2 /* OutlineCode.QUADRATIC */, parseInt(parts[i++]), parseInt(parts[i++]), parseInt(parts[i++]), parseInt(parts[i++]));
                    break;
                case 'b':
                    result.push(3 /* OutlineCode.BEZIER */, parseInt(parts[i++]), parseInt(parts[i++]), parseInt(parts[i++]), parseInt(parts[i++]), parseInt(parts[i++]), parseInt(parts[i++]));
                    break;
            }
        }
        return result;
...
class Glyph extends Element {
    ...
    static renderOutline(ctx, outline, scale, x_pos, y_pos) {
        ...
        while (!go.done()) {
            switch (go.next()) {
                case 0 /* OutlineCode.MOVE */:
                    ctx.moveTo(go.nextX(), go.nextY());
                    break;
                case 1 /* OutlineCode.LINE */:
                    ctx.lineTo(go.nextX(), go.nextY());
                    break;
                case 2 /* OutlineCode.QUADRATIC */:
                    x = go.nextX();
                    y = go.nextY();
                    ctx.quadraticCurveTo(go.nextX(), go.nextY(), x, y);
                    break;
                case 3 /* OutlineCode.BEZIER */:
                    x = go.nextX();
                    y = go.nextY();
