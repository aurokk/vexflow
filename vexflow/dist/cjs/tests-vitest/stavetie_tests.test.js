...
describe('StaveTie', () => {
    ...
    function createTest(notesData, setupTies) {
        return (options) => __awaiter(this, void 0, void 0, function* () {
            ...
            factory.draw();
            yield expectMatchingScreenshot(options, 'stavetie_tests.test.ts');
            assert.ok(true);
        });
    }
    runTest('Simple StaveTie', createTest(['(cb4 e#4 a4)/2, (d4 e4 f4)', { stem: 'down' }], (f, notes) => {
        f.StaveTie({
            from: notes[0],
            to: notes[1],
            first_indices: [0, 1],
    ...
    runTest('Chord StaveTie', createTest(['(d4 e4 f4)/2, (cn4 f#4 a4)', { stem: 'down' }], (f, notes) => {
        f.StaveTie({
            from: notes[0],
            to: notes[1],
            first_indices: [0, 1, 2],
    ...
    runTest('Stem Up StaveTie', createTest(['(d4 e4 f4)/2, (cn4 f#4 a4)', { stem: 'up' }], (f, notes) => {
        f.StaveTie({
            from: notes[0],
            to: notes[1],
            first_indices: [0, 1, 2],
    ...
    runTest('No End Note With Clef', createTest(['(cb4 e#4 a4)/2, (d4 e4 f4)', { stem: 'down' }], (f, notes, stave) => {
    ...
    runTest('No End Note', createTest(['(cb4 e#4 a4)/2, (d4 e4 f4)', { stem: 'down' }], (f, notes) => {
        f.StaveTie({
            from: notes[1],
            first_indices: [2],
            last_indices: [2],
    ...
    runTest('No Start Note With Clef', createTest(['(cb4 e#4 a4)/2, (d4 e4 f4)', { stem: 'down' }], (f, notes, stave) => {
    ...
    runTest('No Start Note', createTest(['(cb4 e#4 a4)/2, (d4 e4 f4)', { stem: 'down' }], (f, notes) => {
        f.StaveTie({
            to: notes[0],
            first_indices: [2],
            last_indices: [2],
    ...
    runTest('Set Direction Down', createTest(['(cb4 e#4 a4)/2, (d4 e4 f4)', { stem: 'down' }], (f, notes) => {
        f.StaveTie({
            from: notes[0],
            to: notes[1],
            first_indices: [0, 1],
    ...
    runTest('Set Direction Up', createTest(['(cb4 e#4 a4)/2, (d4 e4 f4)', { stem: 'down' }], (f, notes) => {
        f.StaveTie({
            from: notes[0],
            to: notes[1],
            first_indices: [0, 1],
