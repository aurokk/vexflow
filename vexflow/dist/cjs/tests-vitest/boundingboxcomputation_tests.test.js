...
import { BoundingBoxComputation, Factory, Glyph } from '../src/index';
 ...
 */
function createContext(backend, elementId, points) {
    let w = points[0];
    let h = points[1];
...
describe('BoundingBoxComputation', () => {
    ...
    quadraticParams.forEach((params, index) => {
        runTest(`Quadratic Test ${index}`, (options, contextBuilder) => __awaiter(void 0, void 0, void 0, function* () {
            const assert = createAssert();
            ...
            const o = [0 /* OutlineCode.MOVE */, x0, -y0, 2 /* OutlineCode.QUADRATIC */, x2, -y2, x1, -y1];
            ...
            ctx.lineTo(x2, y2);
            ctx.stroke();
            yield expectMatchingScreenshot(options, 'boundingboxcomputation_tests.test.ts');
    ...
    cubicParams.forEach((params, index) => {
        runTest(`Cubic Test ${index}`, (options, contextBuilder) => __awaiter(void 0, void 0, void 0, function* () {
            const assert = createAssert();
            const points = params.points;
            const box = params.box;
            const ctx = createContext(options.backend, options.elementId, points);
            ...
            // Regression test for a prior bug: compute the bounding box again,
            // this time using the Glyph.getOutlineBoundingBox code path.
            const o = [0 /* OutlineCode.MOVE */, x0, -y0, 3 /* OutlineCode.BEZIER */, x3, -y3, x1, -y1, x2, -y2];
            const bbox = Glyph.getOutlineBoundingBox(o, 1, 0, 0);
            rect(ctx, '#fa0', 1, bbox.getX(), bbox.getY(), bbox.getW(), bbox.getH());
            // Draw curve.
            ctx.setLineWidth(1);
