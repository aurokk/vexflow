...
const ChordStaveTests = {
  Start(): void {
    ...
    run('ChordStave with Time Signature', withModifiers);
    run('Multiple ChordStaves (4x4 Grid)', multipleStaves);
  },
};
...
function draw(options: TestOptions, contextBuilder: ContextBuilder): void {
  ...
  options.assert.equal(stave.getYForLine(5), 145, 'getYForLine(5)');
  options.assert.equal(stave.getYForLine(0), 70, 'getYForLine(0) - Top Line');
  options.assert.equal(stave.getYForLine(4), 130, 'getYForLine(4) - Bottom Line');

  options.assert.ok(true, 'all pass');
}

function withModifiers(options: TestOptions, contextBuilder: ContextBuilder): void {
  ...
  options.assert.equal(modifiers.length, 1, 'Should have 1 modifier');
  options.assert.ok(true, 'Time signature renders successfully');
}

function multipleStaves(options: any): void {
  ...
  const f = VexFlowTests.makeFactory(options, canvasWidth, 400);
  ...
  for (let row = 0; row < 4; row++) {
    for (let col = 0; col < 4; col++) {
      ...
      const chordNote = new ChordNote({ duration: 'w' }, { line: 2 }).addText(chord.root).addTextSuperscript(chord.ext);
  ...
  options.assert.equal(chordIndex, 16, 'Should render 16 chords');
  options.assert.ok(true, '16 ChordStaves with chords render successfully');
}
