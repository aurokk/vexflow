...
describe('GhostNote', () => {
   ...
   */
  function createTest(addItems: (f: Factory, s: EasyScore) => void, backend: number, elementId: string) {
    const assert = createAssert();
    const factory = makeFactory(backend, elementId, 550);
  ...
  runTest('GhostNote Basic', (options: TestOptions, contextBuilder: ContextBuilder) => {
    ...
    }, options.backend, options.elementId);
  });

  runTest('GhostNote Dotted', (options: TestOptions, contextBuilder: ContextBuilder) => {
    createTest((f: Factory, score: EasyScore) => {
